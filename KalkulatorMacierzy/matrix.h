...
public:
	...
	matrix<TYP> operator*(matrix<TYP> czynnik)
	{
		...
		{
			string error = "Niezgodno�� wymiarow mnozonych macierzy.";
			throw(error);
		}
		...
		for (int i = 0; i < wynik.iloscWierszy(); i++) //Dla ka�dego wiersza wyniku
			for (int x = 0; x < wynik.iloscKolumn(); x++)	//Dla ka�dej kom�rki wyniku
			{
				int y = 0;
	...
	//Odwracanie - niemo�liwe ze wzgl�du na definicj�
